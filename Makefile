...
$(FILESYSTEM)/$(1): $$(LINK_LIBRARY_PATHS) $$($(1)_OBJS) | build $$(SUBDIR_BUILD_DIRS)
	...
	modbin --name="$(1)" --stack=$$(or $$($(1)_STACKSIZE),$$(STACKSIZE)) "$$@" "$$@"
endef
...
$(LAUNCHME): $(LINK_LIBRARY_PATHS) $(LAUNCHME_OBJS) | build $(SUBDIR_BUILD_DIRS)
	...
	modbin --name="$(NAME)" --stack=$(STACKSIZE) "$@" "$@"
else
LAUNCHME_TARGETS =
endif
...
modbin-launchme: $(LAUNCHME)
	modbin --name="$(NAME)" --stack=$(STACKSIZE) "$(LAUNCHME)" "$(LAUNCHME)"
else
modbin-launchme:
endif

modbin-%: $(FILESYSTEM)/%
	modbin --name="$*" --stack=$(or $($*_STACKSIZE),$(STACKSIZE)) "$<" "$<"
...
$(ISONAME): $(LAUNCHME_TARGETS) $(PROGRAM_PATHS) $(COPY_DATA_STAMP) FORCE
ifeq ($(IS_POSIX_SHELL),1)
	mkdir -p "$(ISO_DIR)"
else
	if not exist "$(subst /,\,$(ISO_DIR))" mkdir "$(subst /,\,$(ISO_DIR))"
...
.PHONY: builddir isodir clean distclean launchme programs libraries install install-libs modbin modbin-launchme banner iso run FORCE
